      ...
      {(newForm || editingId) && (
        <div className="bg-card border border-white/8 rounded-xl px-[11px] py-[11px] mb-[7px]">
          <div className="flex flex-col sm:flex-row gap-[7px] items-stretch sm:items-end">
            <input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value, slug: editingId ? f.slug : e.target.value.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') }))} placeholder="Nombre" className="flex-1 rounded-lg bg-surface-1 border border-white/8 px-[9px] py-[6px] text-[11px] text-foreground outline-none focus:border-hype/60 transition-colors font-1" />
            <input value={form.slug} onChange={(e) => setForm((f) => ({ ...f, slug: e.target.value }))} placeholder="Slug" className="flex-1 rounded-lg bg-surface-1 border border-white/8 px-[9px] py-[6px] text-[11px] text-foreground outline-none focus:border-hype/60 transition-colors font-1" />
